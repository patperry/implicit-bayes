...
this model misspecification, the MAP estimator $\hat L_{\eta}$
...
For each value of $s$, we generate the normalized Laplacian,
$\mathcal{L}$, corresponding to the random $s$-swapped grid.  We
compute the $n-1$ nonzero eigenvalues of
$[\Tr(\mathcal{L}^{+})]^{-1} \mathcal{L}^{+}$.  Finally, we perform
$1000$ replicates of the procedure and average the resulting
eigenvalues.  Fig.~\ref{F:interpolate} shows the results of the
simulation.  We can see that when $s = 0$, the top two eigenvalues
...
To construct such a pair, we generate population graph
...
\begin{figure}
    ...
    \subfigure[Relative Frobenius error versus $\eta / \bar \tau$]{
      \makebox{\label{F:estimation-frob}\includegraphics[scale=0.6]{plots/estimation-frob-p100}}
    }
    \subfigure[Scaled optimal regularization parameter $\eta / \bar \tau$ plotted against $m$
    and $s$]{
      \makebox{\label{F:optimal-frob}\includegraphics[scale=0.48]{plots/optimal}}
    }
    %\caption{Analytical and emprical priors}
\end{figure}
