...
\begin{figure}[t]
    ...
    \subfigure[Spectrum of $\Theta$ for grid and expander graphs]{
...
\begin{figure}[t]
    ...
    \subfigure[Optimal regularization parameter $\eta^\ast / \bar \tau$]{
...
Laplacian $\mathcal{L}$ as in Sec.~\ref{S:prior-evaluation}.  Let
$\mu$ be the number of edges in $\mathcal{G}$. The
...
Sec.~\ref{S:estimation}, Fig.~\ref{F:optimal-frob} shows
how $\eta^\ast$, the optimal value of $\eta$, depends on the sampling
proportion $m / \mu$ and the swaps per edges $s / \mu$.
We can see that as the sample size $m$ increases, $\eta^\ast$ converges
...
%\section{Conclusion}
%\label{sxn:conc}
%When use PageRank for local graph partitioning, we are using a \emph{regularized}
%estimate of the pseudoinverse of the Laplacian.  Conversely we can use
%PageRank diffusions to \emph{efficiently} find the MAP estimate of $\mathcal{L}^+$.
