...
$\Theta = \Theta(\mathcal{L}) = \tau^{-1} \mathcal{L}^+$. For
regularized estimate $\mathcal{\hat L}_eta$ we define $\hat \tau_\eta$ and
$\hat \Theta_\eta$ similarly.  For sample Laplacian $L$ we define
unregularized estimates $\hat \tau = \hat \tau(L) = \Tr(L^+)$ and
$\hat \Theta = \hat \Theta(L) = t^{-1} L^+$.
